...
/* jquery-ui-1.11.2.custom/jquery-ui.js */
...
function styleMessage(parentId,messageLocal,response){if(response['type']==='Erro'){$(parentId+' '+messageLocal).addClass('error');}else if(response['type']==='Sucesso'){$(parentId+' '+messageLocal).addClass('success');}else if(response['type']==='Cuidado'){$(parentId+' '+messageLocal).addClass('warning');}}
...
$('#submit-crete-account').click(function(){var url='users/add';var formData=$('#create-account-form').serialize();ajaxJsonData(displayAddMessage,url,formData)});function displayAddMessage(response){var parentId='#create_account_modal';var messageLocal='.message';printMessage(parentId,messageLocal,response);styleMessage(parentId,messageLocal,response);if(response['type']==='Sucesso'){location.reload();}}
$.getJSON("../json/products.json",function(products){autoCompleteMulti(products,'#search');});function autoCompleteMulti(words,selector){$(selector).bind("keydown",function(event){if(event.keyCode===$.ui.keyCode.TAB&&$(this).autocomplete("instance").menu.active){event.preventDefault();}}).autocomplete({minLength:2,source:function(request,response){var results=$.ui.autocomplete.filter(words,extractLast(request.term));response(results.slice(0,10));},focus:function(){return false;},select:function(event,ui){var terms=split(this.value);terms.pop();terms.push(ui.item.value);terms.push("");this.value=terms.join(" ");return false;}});}
function split(val){return val.split(/ \s*/);}
function extractLast(term){return split(term).pop();}
$("#password").popover({title:'A senha deve conter entre 8 e 16 caracteres, incluindo:',content:'<ul><li>Letras Maiusculas</li><li>Letras Minusculas</li><li>Numeros</li></ul>',trigger:'hover',placement:'right'});$("#login-form").validate({rules:{email:{required:true,minlength:8},password:"required"},messages:{email:{required:"Please provide your Login",minlength:"Your Login must be at least 8 characters"},pass:"Please provide your password"}});$('#offers-slide').bxSlider({slideWidth:812,minSlides:1,maxSlides:1,moveSlides:1,auto:true});$('#news-slide').bxSlider({slideWidth:240,minSlides:1,maxSlides:5,moveSlides:1,slideMargin:10,infiniteLoop:false,auto:true,pager:false});trendProductsSliderConfig={slideWidth:250,minSlides:1,maxSlides:1,moveSlides:1,auto:true,pager:false};$('#most-popular-products').bxSlider(trendProductsSliderConfig);$('#best-sellers-products').bxSlider(trendProductsSliderConfig);$('#releases-products').bxSlider(trendProductsSliderConfig);$('#back-top').click(function(){$('html, body').animate({scrollTop:0},'slow');return false;});$('#most-pupular-subcat').change(function(){var selectId='#most-pupular-subcat';var selectContainer='#most-popular-container';var url='products/most-popular';var ulSlider='most-popular-products';changeProducts(selectId,selectContainer,url,ulSlider);});$('#best-sellers-subcat').change(function(){var selectId='#best-sellers-subcat';var selectContainer='#best-sellers-container';var url='products/most-popular';var ulSlider='best-sellers-products';changeProducts(selectId,selectContainer,url,ulSlider);});$('#releases-subcat').change(function(){var selectId='#releases-subcat';var selectContainer='#releases-container';var url='products/most-popular';var ulSlider='releases-products';changeProducts(selectId,selectContainer,url,ulSlider);});function changeProducts(selectId,selectContainer,url,ulSlider){$.ajax({type:'post',url:url,data:{subCategory:$(selectId).val()},beforeSend:function(){$(selectContainer).empty();$(selectContainer).append('<img src="img/loader.gif" class="img-loader">');},success:function(products){$(selectContainer).empty();formatProducts(products,ulSlider,selectContainer);},dataType:'json',global:false});}
function formatProducts(products,ulSlider,selectContainer){var divWrapper=document.createElement('div');$(divWrapper).css({'max-width':'250px','margin':'0px auto'});var divViewport=document.createElement('div');$(divViewport).addClass('bx-viewport');$(divViewport).css({'width':'100%','overflow':'hidden','position':'relative','height':'320px'});var ul=document.createElement('ul');$(ul).attr('id',ulSlider);var productsLength=products.length;for(var i=0;i<productsLength;i++){var li=document.createElement("li");var div=document.createElement("div");$(div).addClass('item-block');var img=document.createElement("img");$(img).attr('alt',products[i]['product_name']);$(img).attr('src','img/'+products[i]['thumbnail']);$(div).append(img);var spanName=document.createElement("span");$(spanName).addClass('item-name');$(spanName).text(products[i]['product_name']);$(div).append(spanName);if(products[i]['price']<products[i]['old_price']){var spanOldPrice=document.createElement("span");$(spanOldPrice).addClass('item-old-price');$(spanOldPrice).text('De R$ '+products[i]['price']);$(div).append(spanOldPrice);}
...
$("#product-img").elevateZoom({galleryActiveClass:'active',gallery:'product-gallery',cursor:'crosshair',zoomType:'inner'});function redirect(option){location=option.value;}
...
$(function(){var legendKeyValue={"VAGA":"#CF000F","CALCADOS":"#19B5FE","COMIDA":"#22313F","VESTUARIO":"#F9690E","ADERECOS":"#2ECC71"};$('#mall-map').vectorMap({map:'mall',backgroundColor:'transparent',series:{regions:[{values:{loja_A:"VAGA",loja_B:"CALCADOS",loja_C:"COMIDA",loja_D:"VESTUARIO",loja_E:"ADERECOS",loja_F:"COMIDA",loja_G:"VAGA",loja_H:"VESTUARIO",loja_I:"VAGA",loja_J:"CALCADOS",loja_K:"VAGA",loja_L:"COMIDA",loja_M:"VESTUARIO",loja_N:"CALCADOS",loja_O:"ADERECOS",loja_P:"VAGA",loja_Q:"CALCADOS",loja_R:"VAGA",loja_S:"COMIDA",loja_T:"VESTUARIO",loja_U:"VAGA",loja_V:"ADERECOS",loja_W:"VAGA",loja_X:"COMIDA",loja_Y:"CALCADOS",loja_Z:"VAGA",loja_1:"VESTUARIO",loja_2:"ADERECOS",loja_3:"CALCADOS",loja_4:"VESTUARIO"},scale:legendKeyValue}]},onRegionTipShow:function(event,label,code){label.html("<img src=\"http://localhost/PROJETOS/ShoppingResources/stores/facade/loja-fachada.png\">"+"<h4>"+label.html()+"</h4>"+"<p>"+"Descricao Descricao Descricao Descricao Descricao "+"</p>"+"<span>"+"Local: A#9"+"</span>");},onRegionClick:function(event,code){location="https://pt.wikipedia.org/";}});createMapLegend(legendKeyValue);});function createMapLegend(legendKeyValue){for(var key in legendKeyValue){var divContainer=document.createElement('div');var divColor=document.createElement('div');$(divContainer).addClass('legend-container');$(divColor).addClass('legend-color');$(divColor).css('background-color',legendKeyValue[key]);$(divContainer).append(divColor);$(divContainer).append('<span> - '+key+'</span>');$('#mall-map-legend').append(divContainer);}}
