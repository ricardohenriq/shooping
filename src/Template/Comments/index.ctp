...
<div class="comments index large-10 medium-9 columns">
    ...
    <thead>
        <tr>
            ...
            <th><?= $this->Paginator->sort('comment_text') ?></th>
    ...
    <?php foreach ($comments as $comment): ?>
        <tr>
            ...
            <td><?= h($comment->comment_text) ?></td>
            <td>
                <?= $comment->has('product') ? $this->Html->link($comment->product->id, ['controller' => 'Products', 'action' => 'view', $comment->product->id]) : '' ?>
            </td>
            <td>
